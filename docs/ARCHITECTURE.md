...
- **`ci.yml`** — on every PR/push to `main`: `npm ci`, `ts:check`, `lint`, `format:check`, `test`, a web export (`expo export --platform web`) as a build sanity check, and `deadcode` (knip).
